...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        if not skip_token_embedding:
            self.to_token_embedding = nn.Linear(token_dim, dim)
            get_pylogger(__name__).info("Embedded Token !!!")
        else:
            self.to_token_embedding = nn.Identity()
            if token_dim != dim:
                raise ValueError(
                    f"token_dim {token_dim} != dim when skip to_token_embedding is true"
                )
    ...
    def forward(self, x: torch.Tensor, *args, context=None, context_list=None):
        ...
        x, token_list = self.transformer(x, *args, context=context, context_list=context_list)
        return x, token_list




class SKELTransformerDecoder(nn.Module):
    def __init__(
        self,
        num_tokens: int,
        token_dim: int,
        ...
        if not skip_token_embedding:
            self.to_token_embedding = nn.Linear(token_dim, dim)
            get_pylogger(__name__).info("to_token_embedding !")
        else:
            self.to_token_embedding = nn.Identity()
            if token_dim != dim:
                raise ValueError(
    ...
    def forward(self, x: torch.Tensor, *args, context=None, context_list=None, 
                poses_init=None, betas_init=None, cam_init=None):
        """
            poses_init: B, 144
            betas_init: B, 10
            cam_init: B, 3
        ...
        """
        assert poses_init is not None and betas_init is not None and cam_init is not None
        if context_list is None:
            context_list = [context] * len(self.layers)
        if len(context_list) != len(self.layers):
            raise ValueError(f"len(context_list) != len(self.layers) ({len(context_list)} != {len(self.layers)})")
