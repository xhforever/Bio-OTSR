...
def hrnet_w32(
    ...
    cfg = get_cfg_defaults(pretrained_ckpt_path, width=32, downsample=downsample, use_conv=use_conv)
    return get_pose_net(cfg, is_train=True)


def hrnet_w48(
        pretrained_ckpt_path='hrnet',
        downsample=False,
        use_conv=False,
):

    cfg = get_cfg_defaults(pretrained_ckpt_path, width=48, downsample=downsample, use_conv=use_conv)
    return get_pose_net(cfg, is_train=True)
