...
class Evaluator:
    ...
    def __call__(self, output: Dict, batch: Dict, opt_output: Optional[Dict] = None):
        ...
        num_samples = pred_keypoints_3d.shape[1] # 1 -> per annotation with repsect to per person
...
class EvaluatorPCK:
    ...
    def compute_pcks(self):
        ...
        # acc -> single keypoints, avg_acc -> avg_acc keypoints
        pcks = [
            # B, num_kp, 3
    ...
    def __call__(self, output: Dict, batch: Dict, opt_output: Optional[Dict] = None):
        ...
        gt_bbox_expand_factor = (batch['bbox_size']/(batch['_scale']*200).max(dim=-1).values)
