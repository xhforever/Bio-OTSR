...
"""
SKEL人体模型实现
生物力学精确的人体骨骼模型，用于HSMR项目中的3D人体重建
...
class ModelOutput:
    ...
    定义人体模型的标准输出格式
    """
    vertices: Optional[Tensor] = None      # 顶点坐标
    joints: Optional[Tensor] = None        # 关节位置
    full_pose: Optional[Tensor] = None     # 完整姿态参数
    global_orient: Optional[Tensor] = None # 全局方向
    transl: Optional[Tensor] = None        # 平移向量
    v_shaped: Optional[Tensor] = None      # 形状变形后的顶点
...
class SKELOutput(ModelOutput):
    """
    SKEL模型专用输出数据类
    继承基础模型输出，添加SKEL特有的输出字段
    """
    betas: Optional[Tensor] = None           # 形状参数
    body_pose: Optional[Tensor] = None       # 身体姿态
    skin_verts: Optional[Tensor] = None      # 皮肤网格顶点
    skel_verts: Optional[Tensor] = None      # 骨骼顶点
    joints: Optional[Tensor] = None          # 关节位置
    joints_ori: Optional[Tensor] = None      # 关节方向
    poses: Optional[Tensor] = None           # 姿态参数
    trans : Optional[Tensor] = None          # 平移参数
    pose_offsets : Optional[Tensor] = None   # 姿态偏移
    joints_tpose : Optional[Tensor] = None   # T-pose关节位置
    v_skin_shaped : Optional[Tensor] = None  # 形状变形后的皮肤顶点
...
class SKEL(nn.Module):
    ...
    生物力学精确的人体骨骼模型，相比传统SMPL模型提供更精确的骨骼结构。
    支持复杂的关节约束和生物力学特性，用于高质量的3D人体重建。
    
    Args:
        gender (str): 性别，'male' 或 'female'
        model_path (str, optional): 模型文件路径
        custom_joint_reg_path (str, optional): 自定义关节回归器路径
    """
    ...
    def forward(self, poses, betas, trans, poses_type='skel', skelmesh=True, dJ=None, pose_dep_bs=True):
        """
        SKEL模型前向传播函数
        从姿态、形状和平移参数生成3D人体网格和关节位置
        
        Args:
            poses (Tensor): 姿态参数，形状为 (B, 46)
            betas (Tensor): 形状参数，形状为 (B, 10)，与SMPL相同
            trans (Tensor): 平移参数，形状为 (B, 3)
            poses_type (str): 姿态类型，默认'skel'，不应更改
            skelmesh (bool): 是否返回骨骼顶点，骨骼网格较重，GPU内存不足时设为False
            dJ (Tensor, optional): 关节位置偏移，形状为 (B, 24, 3)，默认为None
            pose_dep_bs (bool): 是否应用姿态相关的混合形状，默认True
            
        Returns:
            SKELOutput: 包含以下字段的输出对象
                - betas: 形状参数 (B, 10)
                - poses: 姿态参数 (B, 46) 
                - skin_verts: 皮肤网格顶点 (B, 6890, 3)
                - skel_verts: 骨骼顶点
                - joints: 关节位置 (B, 24, 3)
                - joints_ori: 关节方向 (B, 24, 3, 3)
                - trans: 平移参数 (B, 3)
                - pose_offsets: 姿态相关混合形状偏移 (B, 6080, 3)
                - joints_tpose: T-pose下的关节位置 (B, 24, 3)
