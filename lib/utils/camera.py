...
def perspective_projection(
    ...
    '''
    Computes the perspective projection of a set of 3D points.计算一组三维点的透视投影。
    ...
    ### Args
        - points: torch.Tensor, (B, N, 3)
            - The input 3D points.输入的三维点。
        - translation: torch.Tensor, (B, 3)
            - The 3D camera translation.三维相机的平移。
        - focal_length: torch.Tensor, (B, 2)
            - The focal length in pixels.焦距。
        - camera_center: torch.Tensor, (B, 2)
            - The camera center in pixels.相机中心。
        - rotation: torch.Tensor, (B, 3, 3)
            - The camera rotation.相机旋转。

    ### Returns
        - torch.Tensor, (B, N, 2)
            - The projection of the input points.输入点的投影。
    '''
    B = points.shape[0]
    if rotation is None:
    ...
    # Populate intrinsic camera matrix K.填充内参相机矩阵K。
    ...
    # Apply perspective distortion应用透视畸变。
    projected_points = points / points[:, :, -1].unsqueeze(-1)

    # Apply camera intrinsics应用相机内参。
