...
def parse_args():
    ...
    parser.add_argument('--smpl24', action='store_true', help='Use SMPL-24 joints for evaluation (per-joint metrics available).')
...
def get_data(ds_name, cfg):
    ...
    if ds_name in ['MOYO']:
        ...
        # Handle both training config (exp.data) and release config (data)
        data_cfg = cfg.exp.data if hasattr(cfg, 'exp') and hasattr(cfg.exp, 'data') else cfg.data
        data = instantiate(data_cfg, _recursive_=False)
...
def _forward_gt_smpl24(gt):
    ...
    # Accept both 'smpl' and 'smpl_params' keys from dataset
    smpl_args = gt['smpl'] if 'smpl' in gt else gt['smpl_params']
    # Determine device safely
    try:
        dev = smpl_args['betas'].device
    except Exception:
        dev = torch.device('cpu')
    if not hasattr(_forward_gt_smpl24, 'smpl_model'):
        _forward_gt_smpl24.smpl_model = make_SMPL(gender='neutral', device=dev)
    smpl_model = _forward_gt_smpl24.smpl_model.to(dev)
    # SMPL inference.
    smpl_output = smpl_model(**smpl_args)
...
class UniformEvaluator():
    ...
    def __init__(self, data, device='cuda:0'):
        ...
        self.device = device
        # Enable SMPL-24 evaluation mode for standard 3D datasets when requested
        use_smpl24 = getattr(UniformEvaluator, 'USE_SMPL24', False)
        if data['name'] in ['MOYO'] or (use_smpl24 and data['name'] in ['H36M-VAL-P2','3DPW-TEST']):
