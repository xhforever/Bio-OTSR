"""
HSMR演示工具包
提供HSMR模型推理所需的数据预处理、检测、重建和可视化功能
"""
...
# ImageNet预训练模型的标准化参数（RGB格式，0-255范围）
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='HSMR演示脚本 - 从图像或视频重建3D人体')
    parser.add_argument('-t', '--input_type', type=str, default='auto', 
                       help='指定输入类型。auto: 文件→视频, 文件夹→图片', 
                       choices=['auto', 'video', 'imgs'])
    parser.add_argument('-i', '--input_path', type=str, required=True, 
                       help='输入图片文件夹或视频文件路径')
    parser.add_argument('-o', '--output_path', type=str, default=PM.outputs/'demos', 
                       help='输出结果保存路径')
    parser.add_argument('-m', '--model_root', type=str, default=DEFAULT_HSMR_ROOT, 
                       help='包含`.hydra/config.yaml`的模型根目录')
    parser.add_argument('-d', '--device', type=str, default='cuda:0', 
                       help='运行设备（cuda:0或cpu）')
    parser.add_argument('--det_bs', type=int, default=10, 
                       help='人体检测器的最大批次大小')
    parser.add_argument('--det_mis', type=int, default=512, 
                       help='人体检测器的最大图像尺寸')
    parser.add_argument('--rec_bs', type=int, default=300, 
                       help='人体重建的批次大小')
    parser.add_argument('--max_instances', type=int, default=5, 
                       help='每张图片最大处理的人体实例数')
    parser.add_argument('--ignore_skel', action='store_true', 
                       help='不渲染骨骼网格以加速渲染')
    parser.add_argument('--have_caption', action='store_true', 
                       help='在渲染图像上添加标题')
...
def load_inputs(args, MAX_IMG_W=1920, MAX_IMG_H=1080):
    ...
    Args:
        args: 命令行参数对象
        MAX_IMG_W (int): 最大图像宽度
        MAX_IMG_H (int): 最大图像高度
        
    Returns:
        tuple: (raw_imgs, inputs_meta)
            - raw_imgs: 图像列表
            - inputs_meta: 输入元数据字典
    """
    # 1. 推断输入类型
    ...
    if args.input_type != 'auto': 
        ...
        inputs_type = 'video' if Path(args.input_path).is_file() else 'imgs'
    get_logger(brief=True).info(f'🚚 从以下路径加载输入: {inputs_path}, 识别为 <{inputs_type}>')
    ...
    if inputs_type == 'video':
        ...
        raise ValueError(f'不支持的输入类型: {inputs_type}')
    get_logger(brief=True).info(f'📦 总共加载了 {len(raw_imgs)} 张图像')
...
def imgs_det2patches(imgs, dets, downsample_ratios, max_instances_per_img):
    """
    从原始图像和检测结果中提取人体实例的图像块
    
    Args:
        imgs: 原始图像列表
        dets: 检测结果列表
        downsample_ratios: 下采样比例列表
        max_instances_per_img: 每张图片的最大实例数
        
    Returns:
        tuple: (patches, det_meta) 
            - patches: 人体图像块数组
            - det_meta: 检测元数据
    """
