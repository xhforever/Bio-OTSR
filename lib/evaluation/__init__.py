from .metrics import *
from .metrics.utils import L2_error, similarity_align_to, m2mm
...
class HSMR3DEvaluator(EvaluatorBase):
    ...
    def eval(self, **kwargs):
        ...
        # Per-joint errors (in mm) for SMPL-24 path
        try:
            per_joint = L2_error(j3d_pd, j3d_gt) * m2mm           # (..., J)
            j3d_pd_aligned = similarity_align_to(j3d_pd, j3d_gt)  # (..., J, 3)
            per_joint_pa = L2_error(j3d_pd_aligned, j3d_gt) * m2mm # (..., J)

            # Save per-joint and overall averages so downstream JSON can export them
            J = per_joint.shape[-1]
            for j in range(J):
                self.accumulator[f'kp{j}_mpjpe'].append(per_joint[..., j].detach().cpu())
                self.accumulator[f'kp{j}_pa_mpjpe'].append(per_joint_pa[..., j].detach().cpu())
            self.accumulator['kpAvg_mpjpe'].append(per_joint.mean(dim=-1).detach().cpu())
            self.accumulator['kpAvg_pa_mpjpe'].append(per_joint_pa.mean(dim=-1).detach().cpu())
        except Exception:
            pass
