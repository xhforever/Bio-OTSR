...
class Evaluator:
    ...
    def __init__(self,
        ...
        # Per-joint error accumulators (mm), for the selected keypoint subset
        self.num_keypoints = len(self.keypoint_list)
        self.per_joint_mpjpe_sum = np.zeros((self.num_keypoints,), dtype=np.float64)
        self.per_joint_pa_mpjpe_sum = np.zeros((self.num_keypoints,), dtype=np.float64)
        self.per_joint_count = 0
    ...
    def get_metrics_dict(self) -> Dict:
        ...
        if self.pck_evaluator is not None:
            ...
            d1.update(d2)
        # Append per-joint averages (computed from mode sample) in mm
        if self.per_joint_count > 0:
            per_joint_mpjpe_avg = (self.per_joint_mpjpe_sum / self.per_joint_count).astype(float)
            per_joint_pa_mpjpe_avg = (self.per_joint_pa_mpjpe_sum / self.per_joint_count).astype(float)
            # Per-joint keys
            for i, (e_abs, e_pa) in enumerate(zip(per_joint_mpjpe_avg, per_joint_pa_mpjpe_avg)):
                d1[f'kp{i}_mpjpe'] = float(e_abs)
                d1[f'kp{i}_pa_mpjpe'] = float(e_pa)
            # Overall averages across joints
            d1['kpAvg_mpjpe'] = float(per_joint_mpjpe_avg.mean())
            d1['kpAvg_pa_mpjpe'] = float(per_joint_pa_mpjpe_avg.mean())
        return d1
    ...
    def __call__(self, output: Dict, batch: Dict, opt_output: Optional[Dict] = None):
        ...
        # Compute per-joint errors (mode sample only) in mm
        # Select only the required keypoints
        pred_sel = pred_keypoints_3d[:, 0, self.keypoint_list, :]  # (B, K, 3)
        gt_sel = gt_keypoints_3d[:, 0, self.keypoint_list, :]      # (B, K, 3)
        # Absolute per-joint MPJPE
        per_joint_err = torch.sqrt(((pred_sel - gt_sel) ** 2).sum(dim=-1)) * 1000.0  # (B, K)
        # PA per-joint MPJPE
        pred_aligned = compute_similarity_transform(pred_sel, gt_sel)                 # (B, K, 3)
        per_joint_err_pa = torch.sqrt(((pred_aligned - gt_sel) ** 2).sum(dim=-1)) * 1000.0  # (B, K)

        # Accumulate sums across the batch
        per_joint_err_np = per_joint_err.detach().cpu().numpy()
        per_joint_err_pa_np = per_joint_err_pa.detach().cpu().numpy()
        self.per_joint_mpjpe_sum += per_joint_err_np.sum(axis=0)
        self.per_joint_pa_mpjpe_sum += per_joint_err_pa_np.sum(axis=0)
        self.per_joint_count += per_joint_err_np.shape[0]
