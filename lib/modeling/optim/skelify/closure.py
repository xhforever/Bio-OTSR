...
def build_closure(
    ...
    # Shortcuts for the inference of the skeleton model.骨架模型推理的快捷方式
    def inference_skel(inputs):
        ...
        #skel_output.joints: 3D关键点 (B, 44, 3)
        #skel_output.skin_verts: 皮肤网格顶点 (B, 6890, 3)
        return skel_params, skel_output

    # Estimate the camera depth as an initialization if depth loss is enabled.如果深度损失启用，则估计相机深度
    gs_cam_z = None
    ...
    # Prepare the focal length for the perspective projection.准备透视投影的焦距
    ...
    def closure():
        ...
        with PM.time_monitor('reproj'):
            pd_kp2d = perspective_projection(
                    points       = to_tensor(skel_output.joints, device=self.device),# 3D关键点
                    translation  = to_tensor(inputs['cam_t'], device=self.device),# 相机平移
                    focal_length = to_tensor(focal_length_xy, device=self.device),# 焦距
                )
