...
from datasets.constants import SKEL_JOINTS_DICT



def pose_params_to_rot(osim_poses):
    ...
    Nj = 24  # Number of joints in the skeleton
    ...
    start_index = 0
    for i in range(0, Nj):
        joint_object = SKEL_JOINTS_DICT[i]
...
def cam_perspective_projection(points, rotation, translation, cam_intrinsics):
...
def cam_perspective_projection(
    points : torch.Tensor,
...
# xuele-optim
# pass: assert torch.allclose(res_old, res_new, atol=1e-6)
# cost:
#   old: 1.15ms
#   new: 0.24ms
# @torch.compile
