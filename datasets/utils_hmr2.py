...
from util.geometry import axis_angle_to_matrix, euler_angles_to_matrix, matrix_to_euler_angles
...
def gen_trans_from_patch_cv(c_x: float, c_y: float,
    ...
    if scale <= 0 or not np.isfinite(scale):
        print(f"Warning: Invalid scale value {scale}, using 1.0")
        scale = 1.0
    ...
    #     print(f"Warning: NaN or Inf detected in transformation matrix")
    #     print(f"src: {src}")
    #     print(f"dst: {dst}")
    #     print(f"scale: {scale}, rot: {rot}")
...
def get_example_hmr2(img_path: Union[str, np.ndarray], center_x: float, center_y: float,
    ...
    if not return_trans:
        return cvimg, img_patch_cv, img_patch, keypoints_2d, keypoints_3d, smpl_params, has_smpl_params, img_size, augm_record
    else:
        return cvimg, img_patch_cv, img_patch, keypoints_2d, keypoints_3d, smpl_params, has_smpl_params, img_size, trans, augm_record
